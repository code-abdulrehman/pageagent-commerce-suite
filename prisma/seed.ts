...
async function main() {
  ...
  const customerSeeds = new Map<string, string>();
  for (const record of [...mockOrders, ...mockTickets]) {
    customerSeeds.set(record.email, record.customer);
  }

  for (const [email, name] of customerSeeds) {
